...
CREATE VIEW V_MARKS AS SELECT RollNo, Name, Sub_Code, MarksObtained FROM Student JOIN Marks USING(RollNo) WHERE DeptCode = 'CSE';
...
CREATE VIEW V_SUBJECT AS SELECT SubjectCode, SubjectName, Faculty.Name FROM Subject JOIN Faculty ON Subject.Teacher = Faculty.FID WHERE SubjectCode IN (SELECT Sub_Code FROM Marks GROUP BY Sub_Code HAVING COUNT(RollNo) > 1);
...
CREATE VIEW V_STUDENT AS SELECT DISTINCT Name, RollNo, DeptCode FROM Student INNER JOIN Marks USING(RollNo);

SELECT * FROM V_STUDENT;

-- v. Create view V_DEPT with dept code, semester, number of student with more than 75 marks in at least one subject.
CREATE VIEW V_DEPT AS SELECT DeptCode, Semester, COUNT(DISTINCT MarksObtained) AS CountStudents FROM Subject JOIN Marks ON Subject.SubjectCode = Marks.Sub_Code WHERE MarksObtained > 75 GROUP BY DeptCode, Semester;
